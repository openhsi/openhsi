...
# %% auto 0
__all__ = ['CircArrayBuffer', 'CameraProperties', 'DateTimeBuffer', 'DataCube']
...
# %% ../nbs/api/data.ipynb 18
class CameraProperties():
    """Container for OpenHSI camera settings and calibration files."""
    def __init__(self, 
                 json_path:str = None,  # Path to settings file
                 pkl_path:str  = None,  # Path to calibration file
                 print_settings:bool = False, # Print out settings file contents to stdout
                 **kwargs):
    ...
    def dump(self, 
        ...
        """Save the settings json and calibration pickle files."""
...
def tfm_setup(self:CameraProperties, 
              more_setup:Callable[[CameraProperties],None] = None, # callable to add custom setup
              dtype:Union[np.int32,np.float32] = np.int32, # int for digital number and float for radiance/reflectance
              lvl:int = 0, # `processing_lvl` which specifies which predefined processing recipe to use
             ):
...
@patch
def crop(self:CameraProperties, 
         x:np.ndarray,   # raw camera frame
        ) -> np.ndarray: # cropped camera frame
    """Crops to illuminated area"""
    return x[self.settings["row_slice"][0]:self.settings["row_slice"][1],:]

# %% ../nbs/api/data.ipynb 26
@patch
def fast_smile(self:CameraProperties, 
               x:np.ndarray,   # cropped camera frame
              ) -> np.ndarray: # fast smiled corrected camera frame
...
@patch
def fast_bin(self:CameraProperties, 
             x:np.ndarray,   # swath
            ) -> np.ndarray: # spectrally binned swath assuming linear wavelength mapping
...
@patch
def slow_bin(self:CameraProperties, 
             x:np.ndarray,   # swath
            ) -> np.ndarray: # spectrally binned swath using non-linear wavelength mapping
...
@patch
def dn2rad(self:CameraProperties, 
           x:np.ndarray,   # swath in digital numbers Array['λ,x',np.int32]
          ) -> np.ndarray: # swath in radiance Array['λ,x',np.float32]
...
# %% ../nbs/api/data.ipynb 30
@patch
def rad2ref_6SV(self:CameraProperties, 
                x:np.ndarray,   # swath in radiance Array['λ,x',np.float32]
               ) -> np.ndarray: # swath in reflectance Array['λ,x',np.float32]
    """Converts radiance to reflectance using the saved 6SV model."""
...
# %% ../nbs/api/data.ipynb 31
@patch
def set_processing_lvl(self:CameraProperties, 
                       lvl:int = -1, # index to predefined processing recipies
                       custom_tfms:List[Callable[[np.ndarray],np.ndarray]] = None, # override with your own transforms
                      ):
...
# %% ../nbs/api/data.ipynb 34
@patch
def pipeline(self:CameraProperties, 
...
# %% ../nbs/api/data.ipynb 37
class DateTimeBuffer():
    """Records timestamps in UTC time in a circular buffer."""
    def __init__(self, n:int):
        """Initialise a nx1 array and write index"""
        self.data = [datetime.utcnow().replace(tzinfo=None) for i in range(n)]
        self.n = n
        self.write_pos = 0
    ...
    def update(self):
        ...
        self.data[self.write_pos] = datetime.utcnow().replace(tzinfo=None)
        self.write_pos += 1
...
def save(self:DataCube, 
    ...
    if "time" in self.nc: self.nc.time.encoding.pop("_FillValue", None)
