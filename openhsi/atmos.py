...
class Model6SV(object):
    ...
    def __init__(self, 
                 ...
                 atmos_profile:AtmosProfile = AtmosProfile.MidlatitudeSummer, # Py6S atmospheric profile
                 station_num:int = 94299, # radiosonde station number, if None, do not parametrise with sounding data
        ...
        if station_num is not None:
            ...
            radiosonde_url = f"http://weather.uwyo.edu/cgi-bin/sounding?region={region}&TYPE=TEXT%3ALIST&YEAR={z_time.year}&MONTH={z_time.month:02d}&FROM={z_time.day:02d}{z_hour:02d}&TO={z_time.day:02d}{z_hour:02d}&STNM={station_num}"
            try:
                s.atmos_profile = SixSHelpers.Radiosonde.import_uow_radiosonde_data(radiosonde_url,AtmosProfile.MidlatitudeSummer)
            except Exception as e:
                print(f"{e}\nSince server is busy, continue without the radiosonde parameters.")
        # Aerosol
...
@patch
def run_wavelengths(self:Model6SV, wavelengths:np.array, # array of wavelengths in μm
                    n_threads:int = None, # number of threads to run (practically, this equates to number of CPUs)
                   ) -> np.array: # 
...
class ELC(SpectralMatcher):
    ...
    def __init__(self,nc_path:str, # path to saved NetCDF file
                 old_style:bool=False, # option for datacubes saved before first release
                 **kwargs):
...
class DataCubeViewer():
    ...
    def __init__(self,
        ...
        if 'ylabel' in kwargs:
            ...
            if self.dc.dc.data.dtype.type is np.float32:
