...
class SensorStream():
    ...
    def __init__(self, baudrate:int=921_600, # baud rate of serial port
                 port:str="/dev/serial0", # serial port name
                 start_pin:int=17, # GPIO pin of a latching button
                 ssd_dir:str=f"../../../media/pi/fastssd/", # path to save directory
                 cam_name:str=None, # which camera class is being used
                ):
    ...
    def read_packet(self,header:chr = b"*", # start byte of packet
                    num_bytes:int = 76, # number of bytes in packet
                    timeout:float = 2., # timeout in seconds
                   ) -> str: # byte string
